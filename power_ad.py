...
class PowerAnomalyDetector:
 ...
 def predict(self, frame):
  ...
  x=df[cols].astype(float).interpolate(limit_direction='both'); minp=max(5,self.window//3); med=x.rolling(self.window,min_periods=minp).median(); mad=(x-med).abs().rolling(self.window,min_periods=minp).median(); scale=(1.4826*mad).replace(0,np.nan).fillna(x.std().replace(0,1)); residual=(x-med).abs(); score=(residual/scale).max(axis=1).fillna(0); delta=x.pct_change(max(1,self.window//4)).abs().max(axis=1).fillna(0)*100; flag=(score>=self.z_threshold)|(delta>=self.change_threshold); flag=flag.rolling(self.min_duration,min_periods=1).sum()>=self.min_duration; df['anomaly_score']=score.round(3); df['change_pct']=delta.round(3); df['is_anomaly']=flag; df['severity']=np.select([score>=self.z_threshold*2,score>=self.z_threshold,delta>=self.change_threshold],['critical','high','medium'],default='normal'); df['drivers']=residual.fillna(0).idxmax(axis=1); return df
